...
export default function PublicAccountabilityPage() {
  ...
  useEffect(() => {
    async function fetchPublicProfile() {
      // Safely extract the username from the URL using Next.js 16 standards
      if (!params?.username) return;
      
      const targetUsername = decodeURIComponent(params.username as string);

      // We use .ilike instead of .eq to ignore uppercase/lowercase differences
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center text-[#D4AF37] font-mono uppercase tracking-widest">Locating Athlete...</div>;
  }
  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex flex-col items-center justify-center text-[#FAFAFA] font-sans p-6">
        ...
        <p className="text-[#2A2A2A] text-xs mt-4">Searched for: {decodeURIComponent(params?.username as string)}</p>
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-[#FAFAFA] flex flex-col items-center justify-center font-sans p-6 selection:bg-[#D4AF37] selection:text-black">
      <div className="w-full max-w-2xl border border-[#2A2A2A] bg-[#141414] p-8 md:p-12 shadow-2xl text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="border border-[#2A2A2A] bg-[#0A0A0A] p-8">
            <h3 className="text-xs font-mono text-[#8B8B8B] uppercase mb-2 tracking-widest">Current Active Streak</h3>
            <div className="text-7xl font-bold text-white mb-2">{profile.current_streak}</div>
            <p className="text-xs text-[#D4AF37] uppercase tracking-wider">Consecutive Days</p>
          </div>

          <div className="border border-[#2A2A2A] bg-[#0A0A0A] p-8">
            <h3 className="text-xs font-mono text-[#8B8B8B] uppercase mb-2 tracking-widest">All-Time Longest</h3>
            <div className="text-7xl font-bold text-[#8B8B8B] mb-2">{profile.longest_streak}</div>
            <p className="text-xs text-[#8B8B8B] uppercase tracking-wider">Consecutive Days</p>
          </div>
        </div>
      </div>
    </main>
